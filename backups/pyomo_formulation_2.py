...
LR_data = {p: df_lr[df_lr["material_cd"] == p]["line_rate_per_day"].to_numpy()[0]*7/cf for p in model.P}
...
def constraint_5(model,p,t):
    lhs = pyo.quicksum(model.a_pt[p,i] - model.D_pt[p,i] for i in np.array(model.T)[:(t+1)]) + model.BINV[p] - model.SS[p]
...
obj_expr = pyo.quicksum(
    model.COT_pq[p,q]*model.W_pqt[p,q,t]*(1/(7*24))*model.LR_p[p]*gross_margin[p] for p in model.P for q in model.P for t in model.T) + \
    pyo.quicksum(
...
time_sec = 1*60*60
solver.options['seconds'] = time_sec*(n_threads-1)
