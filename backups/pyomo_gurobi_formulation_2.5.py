...
actual_production = {
    (p,t): 0 if (len(df_act_production[(df_act_production["material_cd"] == p) & (df_act_production["week_nbr"] == t)]) == 0) 
    else df_act_production[
        (df_act_production["material_cd"] == p) & 
        (df_act_production["week_nbr"] == t)]["actual_production"].to_numpy()[0]/cf 
    for p in model.P for t in model.T}
...
model.production_non_AL = pyo.Param(model.P,model.T, initialize = actual_production)
...
model.a_pt = pyo.Var(model.P,model.T,domain = pyo.NonNegativeReals)
...
def constraint_1c(model,p,q,t):
    ...
    return lhs <= rhs
model.c_1c = pyo.Constraint(model.P,model.P, model.T_minus_1, rule=constraint_1c)
...
def constraint_3(model,t):
    ...
    return lhs >= rhs
model.c_3 = pyo.Constraint(model.T_minus_1,rule = constraint_3)

def constraint_4(model,p,t):
    lhs = model.a_pt[p,t]
    rhs = model.LR_p[p]*7*model.X_pt[p,t]
    return lhs <= rhs
model.c_4 = pyo.Constraint(model.P,model.T,rule = constraint_4)

def constraint_5(model,p,t):
    lhs = pyo.quicksum(model.a_pt[p,i] for i in np.array(model.T)[:(t+1)]) +\
...
obj_expr = pyo.quicksum(
        ...
        for p in model.P for q in model.P for t in model.T) + \
    pyo.quicksum(
...
# Save Model-----------------------------------------------------------------------------------------------------------
model.write("F23_AL_Chewy_Processing_week_granularity.lp",io_options={'symbolic_solver_labels': True})
...
df_a = pd.DataFrame(columns = model.T, index = model.P)
for i in model.a_pt:
...
df_W = df_W[["from_material_cd","to_material_cd","from_product_name","to_product_name","week_start_date","CO_Flag","CO_Time"]]
print(df_W)
...
df_mps_inventory["inventory"] = df_mps_inventory["safety_stock"] + df_mps_inventory["inventory_overflow_plan"] +\
